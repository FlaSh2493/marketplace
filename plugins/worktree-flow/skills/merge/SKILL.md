...
STEP 0: WIP 커밋
  STEP 1 dry-run으로 머지 대상 워크트리 목록 파악 후, 각 워크트리별:
    `cd {워크트리경로} && git status --porcelain` 실행
    변경사항 없으면: 스킵
    변경사항 있으면:
      1. `cd {워크트리경로} && git diff` 및 변경 파일 목록 분석
      2. 변경사항을 논리적 단위로 그룹핑 (기능별, 레이어별 등)
      3. 각 그룹별로 순서대로:
         `cd {워크트리경로} && git add {해당파일들} && git commit -m "wip({이슈키}): {단위 설명}"`
      커밋 메시지 prefix는 반드시 `wip({이슈키}):` 사용 — 최종 메시지는 squash 시 결정
