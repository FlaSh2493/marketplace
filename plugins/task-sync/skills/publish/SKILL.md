...
STEP 1: 미게시 이슈 목록 출력
  `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/list_tasks.py --state UNPUBLISHED`
  결과 없음 → "게시할 이슈 없음" [STOP]
  각 이슈 검증:
...
STEP 3: Jira Story 생성
  각 이슈:
    3-1. `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/md_to_adf.py {이슈키} "$(pwd)/{task_dir}/{이슈키}/{이슈키}.md" > /tmp/{이슈키}_adf.json`
         exit 1 → reason 출력, 해당 이슈 [SKIP]
    3-2. `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/jira_create.py --project {project_key} --summary "{제목}" --adf-file /tmp/{이슈키}_adf.json {--epic ...} {--sprint ...}`
         exit 0 → key 보관
         exit 1 → reason 출력, 해당 이슈 [SKIP]
    3-3. `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/transition.py {이슈키} NONE PUBLISHED`
