...
description: 이슈키를 지정하여 Jira 이슈를 로컬 마크다운으로 저장하는 스킬. fetch가 내부적으로 호출하거나 직접 사용할 수 있다.
---
...
STEP 1: fetch_write.py 실행
  `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/fetch_write.py {이슈키들} --task-dir {task_dir} --state-dir {state_dir}`
