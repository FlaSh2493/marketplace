...
1. **MCP 재조회** (활성 티켓 전체):
   ...
   - `.docs/{workspace}/_cache/{KEY}.latest.json` 저장
...
5. **정밀 diff 생성** (CLI, 토큰 0):
   ...
   ```
   `.docs/{workspace}/_cache/{KEY}.diff.json` 저장
...
## 출력
- `.docs/{workspace}/_cache/{KEY}.latest.json`
- `.docs/{workspace}/_cache/{KEY}.diff.json` (변경 시)
- `.docs/{workspace}/_cache/{KEY}.snapshot.json` (재PIN 시 pinned_at 갱신)
- `.docs/{workspace}/{domain}/{KEY}.requirement.yaml` (반영 시)
