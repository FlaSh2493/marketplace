...
`/autopilot:merge feat/PLAT-sprint3`을 실행하면:
...
```
/autopilot:init               # 최초 1회: 의존 플러그인 확인 및 그래프 빌드

/autopilot:plan PLAT-101      # 워크트리 생성 → 플랜 수립 → 사용자 승인 → 구현
/autopilot:plan PLAT-102      # 동시에 다른 이슈도 작업 가능 (별도 워크트리)

/autopilot:status             # 활성 워크트리 현황 확인

/autopilot:merge feat/sprint3     # 단일 이슈 squash merge (워크트리 내부에서 실행)
/autopilot:merge-all feat/sprint3 # 전체 이슈 일괄 merge (메인 세션에서 실행)
```
...
/autopilot:restore PLAT-101
→ archive/feat/sprint3/PLAT-101-wip-날짜 태그 목록 표시
→ 선택한 태그에서 브랜치 생성 + git worktree add
→ /autopilot:plan PLAT-101 으로 재작업
→ /autopilot:merge feat/sprint3 으로 재머지
...
/plugin install autopilot@<marketplace-name>
/autopilot:init
```
