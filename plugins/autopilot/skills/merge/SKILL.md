...
- `/autopilot:merge [{워크트리브랜치}]` — 워크트리명 지정 또는 목록에서 선택
  - 타겟 브랜치는 `.autopilot`의 `base_branch` 에서 자동 감지
  - `base_branch`가 `develop/main/...` 계열이면 → 케이스 1 (워크트리 → 타겟)
  - `base_branch`가 피처 브랜치면 → 케이스 2 (타겟 → 워크트리 싱크)
...
STEP 4: 머지 실행
  ...
  [충돌 해결 프로세스] — 머지 충돌은 root_path(타겟 브랜치 체크아웃 상태)에서 발생
    Read(`${CLAUDE_PLUGIN_ROOT}/skills/_shared/CONFLICT_RESOLUTION.md`) 후 절차를 따른다.
...
## [케이스 2] 타겟브랜치 → 워크트리 (PR 전 싱크)

STEP C2-1: 미커밋 변경사항 커밋
  케이스 1 STEP 1과 동일 프로세스

STEP C2-2: 타겟브랜치 fetch + merge
  `cd {worktree_path} && git fetch origin {target_branch}` 실행
  실패 시: 에러 출력 후 [STOP]
  `cd {worktree_path} && git merge origin/{target_branch}` 실행
