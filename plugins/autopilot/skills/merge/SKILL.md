...
- `status == "error"`:
  - `reason == "WORKTREE_NOT_FOUND"`: `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/list_worktrees.py` 실행 후 목록 제시, AskUserQuestion으로 선택. 선택된 경로로 다시 `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/resolve_worktree.py {path}` 실행하여 컨텍스트 확보.
...
- `status == "committed"`: 자동 커밋 완료, STEP 2로 이동.
- `status == "empty"`: 변경사항 없음, STEP 2로 이동.
- `status == "needs_manual"`: 아래 수동 커밋 절차 진행.
...
**STEP C2-2.5: 충돌 사전 검사 (Dry-run)**

```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/merge/scripts/merge_worktrees.py '{target_branch}' --dry-run --branch '{worktree_branch}'
```

- `status == "ok"`: `merge_order` 목록과 `conflict_count` 확인.
- `conflict_count > 0` 이면 `conflict_files` 목록을 표시하고 사용자에게 주의 알림.
