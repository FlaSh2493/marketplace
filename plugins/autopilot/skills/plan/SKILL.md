...
```
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/load_issue.py {이슈키} --sections 목적,요구사항,완료 기준,선행 이슈
```
성공: 내용을 컨텍스트로 보관
실패: reason 출력 후 [STOP]
...
### api 자동 추출
이슈 설명에서 도메인 키워드(명사, 기능명) 추출 → 아래 패턴으로 검색:
```
cd {data.worktree_path} && rg -n "(fetch|axios|apiClient|useMutation|useQuery|useInfiniteQuery)\(" --type ts -l | head -20
```
검색 결과 파일 중 키워드와 관련된 파일만 Read → 실제 호출되는 엔드포인트 경로 수집.
수집된 엔드포인트를 `api` 필드 값으로 사용.

### states 자동 추출
동일 키워드로 상태관리 패턴 검색:
```
cd {data.worktree_path} && rg -n "(useStore|create\(|atom\(|useState|useReducer|useContext)" --type ts -l | xargs rg -l "{키워드}" 2>/dev/null | head -10
```
관련 store/hook 파일명과 사용되는 상태명 수집 → `states` 필드 값으로 사용.
...
**3. 이미지 분석** (이미지 제공 시) — 디자인시스템 컴포넌트 매핑, 레이아웃 구조, spacing/color 토큰을 플랜에 명시. (예: `버튼 → <Button variant='primary'>`, `간격 → gap-4`)
