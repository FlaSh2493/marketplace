...
```
성공: `data.issues`, `data.branch`, `data.branches`(multi), `data.mode`, `data.flags` 보관
실패(exit 1): reason 출력 후 [STOP]

mode 값: `single` | `multi` | `no-spec` | `no-issues`(이슈키 없음 → list_issues.py 호출)
...
1. `data.mode == "no-issues"` 이면:
   ...
   ```
   `status: ok` → `data.display` 출력 후 AskUserQuestion("작업할 이슈키를 선택하세요 (여러 개면 공백으로 구분):\n{data.display}")
   `status: empty` 또는 `status: error` → AskUserQuestion("작업할 이슈키를 입력하세요 (여러 개면 공백으로 구분):")
...
| **plan.md** 쓰기 | `{data.issue_doc_root}/tasks/{이슈키}/plan.md` |
...
### 6. 구현 순서
이슈 간 의존 관계가 있으면 선행 이슈 먼저 구현하도록 순서 명시. Phase 내에서도 작업 순서는 의존성 순서를 따른다.
...
**파일 경로**: `{data.issue_doc_root}/tasks/{이슈키}/plan.md`
예) `이슈키 = SPT-3711` → `{issue_doc_root}/tasks/SPT-3711/plan.md`
디렉토리가 없으면 생성: `mkdir -p {data.issue_doc_root}/tasks/{이슈키}`
...
5. plan.md 작성:
   경로: {issue_doc_root}/tasks/{이슈키}/plan.md
   디렉토리 없으면: mkdir -p {issue_doc_root}/tasks/{이슈키}
...
모든 Agent가 완료되면 결과를 수집하여 완료 마커를 기록한다:
  ```bash
  python3 ${CLAUDE_PLUGIN_ROOT}/scripts/state_manager.py reset plan
  python3 ${CLAUDE_PLUGIN_ROOT}/scripts/state_manager.py mark plan
  ```
