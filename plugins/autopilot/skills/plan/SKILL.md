---
name: autopilot-plan
description: 워크트리를 생성하고 이슈 명세를 로드하여 플랜을 수립한 뒤 tasks/{이슈키}/plan.md 로 저장한다. 실제 구현은 별도 /autopilot:build 에서 수행한다. --no-spec 플래그를 사용하면 명세 로드 없이 사용자 요구사항 기반으로 플랜을 작성한다.
---
...
이 스킬은 **플랜 수립까지만** 담당한다. 실제 코드 편집은 하지 않는다. 완료 시 `{issue_doc_root}/tasks/{이슈키}/plan.md` 를 생성하고 `/autopilot:build` 로 넘긴다.

## 사용법
```
/autopilot:plan [-b {브랜치명}] {이슈키}   ← -b로 브랜치명 직접 지정 가능
/autopilot:plan {이슈키} [--no-spec]       ← --no-spec: 명세 없이 요구사항 기반 작업
```

---

## STEP 0.5: 프로젝트 커스텀 지침 참조 (Memory Handoff)

[_shared/CUSTOM_INSTRUCTIONS.md](../_shared/CUSTOM_INSTRUCTIONS.md)에 따라 다음 명령을 실행하여 프로젝트 지침을 확인한다.

```bash
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/load_custom_instructions.py --all
```

- **필수 참조**: 출력된 모든 지침을 **본 스킬 및 이후 구현 과정의 필수 제약 사항**으로 인식한다.
- **기억 전이**: 이후 단계(build 등)에서도 이 지침들이 유지될 수 있도록, 출력된 내용을 `plan.md`의 `## 프로젝트 커스텀 지침` 섹션에 그대로 기록한다.

---

**인수 파싱:**
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/parse_plan_args.py {args}
```
성공: `data.issue`, `data.branch`, `data.mode`, `data.flags` 보관
실패(exit 1): reason 출력 후 [STOP]

mode 값: `single` | `no-spec` | `no-issues`(이슈키 없음 → list_issues.py 호출)

---

## 전제조건 — 워크트리 진입 (완료 전까지 아래로 절대 넘어가지 않는다)

1. `data.mode == "no-issues"` 이면:
   ```bash
   python3 ${CLAUDE_PLUGIN_ROOT}/scripts/list_issues.py
   ```
   `status: ok` → `data.display` 출력 후 AskUserQuestion("작업할 이슈키를 선택하세요:\n{data.display}")
   `status: empty` 또는 `status: error` → AskUserQuestion("작업할 이슈키를 입력하세요:")
   입력받은 값으로 `parse_plan_args.py` 재실행 → data 갱신

2. 워크트리 생성:

   브랜치명 = `-b` 지정값 또는 `feat/{이슈키소문자, 특수문자→하이픈}`
   ```bash
   python3 ${CLAUDE_PLUGIN_ROOT}/scripts/ensure_worktree.py {브랜치명} --issue {data.issue}
   ```
   성공: data.display를 사용자에게 그대로 출력. `data.worktree_path`, `data.branch`, `data.issue_doc_root`, `data.base_branch`, `data.issue` 보관
   실패: reason 출력 후 [STOP]
...
| **plan.md** 쓰기 | `{data.issue_doc_root}/tasks/{data.issue}/plan.md` |
...
`--no-spec` 없으면:
```
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/load_issue.py {data.issue} --sections 목적,화면/디자인,컴포넌트 힌트,영향 범위,완료 조건,선행 이슈,추가 요구사항,사용 API 목록
```
성공: 내용을 컨텍스트로 보관
실패: reason 출력 후 [STOP]
...
plan.md는 작업을 **Phase 단위로 묶어** 구성한다. 이는 build 단계에서 중간 검증 지점을 확보하고, 논리적으로 연관된 작업을 한 덩어리로 처리하기 위함이다.

- **필수 구성**: plan.md는 항상 1개 이상의 Phase를 가진다.
- **논리적 그룹화**: 레이어(entities/features/shared)나 기능 단위로 묶는다.
  - 예: "API 추가 + 타입 정의" (Phase 1), "훅 + 컴포넌트" (Phase 2), "UI 통합" (Phase 3)
- **분할 기준**: 아래 경우 Phase 분할을 강력히 고려한다.
  - 한 Phase의 대상 파일이 5개를 초과하는 경우
  - 서로 다른 도메인이나 레이어에 걸쳐 대규모 수정이 발생하는 경우
  - 명확한 선행/후행 의존 관계가 있는 경우
- **완료 기준 명시**: 각 Phase는 "빌드 통과", "타입 체크 통과", "특정 UI 렌더링 확인" 등 한 줄로 검증 가능한 **완료 기준**을 포함한다.

### 5. 구현 순서
Phase 내에서도 작업 순서는 의존성 순서를 따른다.
...
## 프로젝트 커스텀 지침
(STEP 0.5에서 로드된 지침이 있다면 여기에 그대로 기록하여 build 등에 전달한다. 없으면 섹션 생략.)
...
| 파일 | 변경 | Phase |
|------|------|-------|
| src/entities/cart/api.ts | 수정 | 1 |
| src/features/cart/hooks/useCartSubmit.ts | 신규 | 2 |

## Phases

### Phase 1: {한 줄 목표 — 이 phase에서 달성하는 산출물}
- 목적: {이 phase가 완료되면 검증 가능한 상태가 무엇인가}
- 대상 파일:
  - src/entities/cart/api.ts (수정)
  - src/entities/cart/types.ts (신규)
- 작업:
  1. POST /api/cart 엔드포인트 함수 추가
  2. CartSubmitRequest/Response 타입 정의
- 완료 기준: entities 레이어에서 API 호출 단위 테스트 가능
...
플랜 수정이 필요하면:
  ...
  - /autopilot:plan {data.issue} (재탐색 포함 재생성)
```
