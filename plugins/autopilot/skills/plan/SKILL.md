...
2. 워크트리 생성:
   ...
   **이슈 1개인 경우:**
   ...
   mkdir -p "$state_dir"
   rm -f "$state_dir/plan" "$state_dir/build" "$state_dir/check" "$state_dir/check-all" "$state_dir/merge" "$state_dir/merge-all" "$state_dir/pr" "$state_dir/review-fix"
   ```
...
모든 Agent가 완료되면 결과를 수집하여 각 브랜치별 완료 마커를 기록한다:
  각 성공한 이슈의 data.branch마다:
  ```bash
  state_dir="$(git rev-parse --show-toplevel)/.docs/task/{data.branch}/.state"
  mkdir -p "$state_dir"
  rm -f "$state_dir/plan" "$state_dir/build" ... (전체 스킬 목록)
  # Write: {state_dir}/plan (빈 파일)
  ```
