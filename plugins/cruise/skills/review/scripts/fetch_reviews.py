#!/usr/bin/env python3
"""
PR의 CodeRabbit 리뷰 코멘트를 수집하고 심각도를 분류한다.
Usage: python3 fetch_reviews.py {owner_repo} {pr_number} {root} [--pushed-at {ISO8601}]
"""
import argparse
import json
import os
...
def is_outdated(path, root):
    _, _, rc = run(f"git ls-files --error-unmatch '{path}'", cwd=root)
    return rc != 0
...
def main():
    ...
    def is_resolved(comment):
        return (comment.get("reactions", {}).get("+1", 0) or 0) > 0
    ...
    for c in root_comments.values():
        if is_resolved(c):
            continue
        path = c.get("path", "")
        if path and is_outdated(path, root):
            continue
    ...
    has_reviews = bool(cr_reviews or cr_issue_comments)
    ...
    is_in_progress = any(
        "review in progress" in body.lower() for body in review_bodies
    )

    new_since_push = False
    if pushed_at:
        new_since_push = any(c.get("created_at", "") > pushed_at for c in cr_comments) or any(
