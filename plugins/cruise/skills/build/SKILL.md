...
4. **판별 4분기**(§2) → 국소 계획: 요구사항 문장 · 작업 항목 · `생성/수정 파일` · **검증 방법**.
   R 문장은 `references/wording.md` 를 따른다 (Jira 댓글에 그대로 실린다). 기술 서술은 작업 항목에 둔다.
...
> **문장은 `references/wording.md` 를 따른다.** 읽는 사람은 이 코드를 안 쓴 동료·리뷰어·배포
> 담당·기획자다. 합니다체로, 밖에서 보이는 변화로, 전문용어는 풀어서 쓴다.
> `## 변경 파일` 의 경로는 그대로 두고 **설명만** 평이하게 쓴다.
...
- `## 변경 파일` — 작업단위(도메인/모듈)별로 `### 그룹명` 서브헤딩을 붙이고, 그 아래 파일당 한 줄로 "무엇이 왜 바뀌었나" (name-status의 A/M/D 반영). 예:
  ```
  ### GA4 이벤트 큐 워커
  - workers/ga4-queue.ts (신규) — 쌓인 이벤트를 순서대로 처리합니다
  - lib/retry.ts (신규) — 실패하면 간격을 늘려가며 다시 시도합니다
  ```
